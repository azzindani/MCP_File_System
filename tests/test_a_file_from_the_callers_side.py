...
class TestAShareLinkIsReadAsTheFile:
    ...
    def test_the_fetch_uses_the_rewritten_address(self, fetching, http_url, monkeypatch):
        ...
        monkeypatch.setattr(
            exchange, "direct_download_url", lambda u: f"{http_url}/data.csv" if u == asked else u
        )
...
class TestAWebPageIsNotAFile:
    ...
    def test_a_share_link_that_serves_a_page_says_it_is_not_public(
        self, fetching, http_url, monkeypatch
    ):
        asked = "https://drive.google.com/file/d/XYZ/view"
        monkeypatch.setattr(
            exchange, "direct_download_url", lambda u: f"{http_url}/uc" if u == asked else u
        )
...
class TestAPathOnTheCallersSide:
        ...
        [
            ...
            pytest.param(
                ...
                marks=pytest.mark.skipif(
                    sys.platform == "darwin", reason="a server on macOS has /Users itself"
                ),
            ),
            pytest.param(
                ...
                "Windows",
                marks=pytest.mark.skipif(
                    os.name == "nt", reason="a server on Windows has drives itself"
                ),
            ),
            pytest.param(
                "D:/exports/sales.csv",
    ...
    @pytest.mark.parametrize(
        "raw", ["Ad_Data.csv", "/workspace/data/Ad_Data.csv", "/etc/hostname", "data/mnt/data/x"]
    )
