...
def _exchange_code(
    client: TestClient, code: str, redirect_uri: str = REDIRECT_URI, **extra
) -> dict:
...
class TestAuthorize:
    ...
    def test_post_wrong_key_rejected(self, tmp_path):
        ...
        resp = client.post(
            "/oauth/authorize",
            data={"api_key": "wrong", "client_id": client_id, "redirect_uri": REDIRECT_URI},
        )
...
class TestTokenExchange:
    ...
    def test_garbage_refresh_token_rejected(self, tmp_path):
        client, _ = _make_client(tmp_path)
        resp = client.post(
            "/oauth/token", data={"grant_type": "refresh_token", "refresh_token": "garbage"}
        )
        assert resp.status_code == 400
