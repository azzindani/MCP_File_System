...
class TestAFileSentInParts:
    ...
    def _parts(
        self, n: int = 3, name: str = "big.csv", whole: bytes | None = None, digest: str = ""
    ):
        import hashlib
    ...
    def _tool(self):
        ...
        accept_inline_files(
            SimpleNamespace(_tool_manager=SimpleNamespace(_tools={"read_it": tool}))
        )
        return tool.fn, ran
    ...
    def test_a_malformed_part_is_refused_by_name(self, inbox, header, says):
        call, _ = self._tool()
        result = call(
            file_path=f"data:text/csv;name=x.csv;{header};base64,{base64.b64encode(b'a').decode()}"
        )
