...
Lives next to server.py, not in shared/ — shared/ and engine.py should stay
free of mcp.* imports (STANDARDS.md engine/server split); this module needs
them, so it stays colocated with the one file that already imports mcp.*.
"""
...
def build_auth(
    ...
    return _StaticTokenVerifier(named), AuthSettings(
        issuer_url=base_url, resource_server_url=base_url
    )
